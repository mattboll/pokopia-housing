...
src/
  algorithm/          # Housing optimization logic
    ...
    cover.js          #   Exact minimum category cover of a house (shopping list)
    items.js          #   Greedy item suggestions over the item catalog
...
public/
  ...
  data/items.json         # Item catalog (generated by npm run sync-items)
  data/items-overrides.json  # Community corrections to the item catalog
...
scripts/
  ...
  sync-serebii.js        # Pokemon data sync from Serebii (npm run sync)
  sync-items.js          # Item catalog sync from Serebii (npm run sync-items)
...
- Item catalog: edit `public/data/items-overrides.json` (add an item, add/replace categories, or `"remove": true`), then run `npm run sync-items`
